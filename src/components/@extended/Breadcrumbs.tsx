...
import navigation from '../../utils/menu-items/menuItems';
    ...
    navigation?.items?.map((menu: NavItemType) => {
      if (menu.type && menu.type === 'group') {
        if (menu?.url && menu.url === customLocation) {
          ...
          getCollapse(menu as { children: NavItemType[]; type?: 'collapse' | 'item' | 'group' });
        }
      }
      return false;
    });
    ...
    if (!custom && menu.children) {
      menu.children.filter((collapse: NavItemType) => {
        if (collapse.type && collapse.type === 'collapse') {
          getCollapse(collapse as { children: NavItemType[]; type?: 'collapse' | 'item' | 'group' });
